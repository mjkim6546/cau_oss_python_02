...
while True:
    ...
    if op == "end":
        ...
        num1,num2,result = arithmetic_ops(add)
    elif op == "-":
        num1,num2,result = arithmetic_ops(sub)
    elif op == "*":
        num1,num2,result = arithmetic_ops(lambda x,y:x*y) # 익명함수(lambda) 사용
    elif op == "/":
        num1,num2,result = arithmetic_ops(lambda x,y:x/y)
    elif op == "%":
        num1,num2,result = arithmetic_ops(lambda x,y:x%y)
    else:
        print("Invalid operation")
        continue # Invalid operation이므로 연산결과를 출력하지 않고 "넘어간다".
    print(f"{num1}{op}{num2} = {result}")  # 연산 결과를 출력
